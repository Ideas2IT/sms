...
def load_actions
  ...
  name_arr=['ADD_USER_TO_GROUP_FROM_ADMIN','LIST_ALL_USERS','SEND_MESSAGE_TO_GROUP','MUTE','REJOIN','INVITE_USERS_TO_GROUP','UNSUBSCRIBE','REMOVE','HELP','GROUP','NICK']
  description_arr=['Creating a new Group','Listing Users of a Group ','Messages sent by users of a Group','Temporary Disconnection','Joining after temporary Disconnection','Inviting Users to join in to a Group','Removing a User from a Group','Removing a group from Subscription','suggests user with the list of available keyword','lists the groups which the user is a member of','nick name of the user']
  keywords_arr=['add','list','msg','mute','rejoin','invite','unsub','rmv','help','group','nick']
